...
export class ReportService {
  async triggerReportGeneration(userId) {
  ...
  async processGeneratedData(taskId, userId, jobId) {
    // 1. Atualiza o status para em progresso
    await prisma.task.update({
      where: { id: taskId },
      data: { status: 'in_progress', description: 'Buscando dados e gerando PDF...' }
    });

    // 2. Busca os clientes no banco
    const clientes = await prisma.client.findMany({ where: { userId } });
    console.log(`[Job ${jobId}] Banco lido! Total de clientes: ${clientes.length}`);
